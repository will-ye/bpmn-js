...
export default function BpmnCopyPaste(bpmnFactory, eventBus, moddleCopy) {
  ...
  eventBus.on('copyPaste.pasteElement', function(context) {
    ...
    // copy boundary event hosts
    var cachedHost = cache[ descriptor.host ];
    if (descriptor.host && cachedHost) {
      var newHostBO = getBusinessObject(cachedHost);
      var newBoundaryEvent = getBusinessObject(descriptor);
      if (newHostBO && newBoundaryEvent) {
        newBoundaryEvent.attachedToRef = newHostBO;
      }
    }
  });
